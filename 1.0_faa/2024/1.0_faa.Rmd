...
#### Validation 1
```{r plot_modvalid1, fig.width=4, fig.height=4}
...
#### Validation 2
```{r plot_mod_valid2, fig.width=9, fig.height=4}
...
### FAA raw
```{r plot_faaraw, fig.width=8, fig.height=5}
ggplot(faa[faa$n>=minf,],aes(x=year,y=fec/1000))+
    geom_point()+
    geom_line()+
    facet_wrap(~ageplus) + xlab("Année | Year") + ylab("Fécondité | Fecundity ('000)")
```
...
```{r plot_gsiyear, fig.width=8, fig.height=5}
...
### Resid dot
```{r faa_res2, message = F, fig.height = 5,fig.width = 10,results = 'hide',message=FALSE}
...
p  + labs(x = 'Year', y = 'Fecundity (thousands)', col = 'Age')
ggsave(filename = paste0('../../img/' ,this.year,'/faa',this.year,'.png'),units = 'cm',height = 8,width = 14)
